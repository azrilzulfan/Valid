// src/components/valid/UserSidebar.tsx
import {
  LayoutDashboard,
  FolderOpen,
  Mic,
...
export function UserSidebar({ activePath }: UserSidebarProps) {
  ...
  const role =
    user?.role === "verifier" ? "Verifikator" : user?.role === "admin" ? "Admin" : "Pencari Kerja";
      ...
      <div className="fixed md:relative bottom-0 left-0 w-full md:w-[240px] h-[75px] md:h-screen bg-[var(--card-bg)] md:border-r-[2.5px] border-t-[2.5px] md:border-t-0 border-[var(--border-color)] flex md:flex-col z-50 md:z-auto transition-all shrink-0 shadow-[0_-8px_20px_rgba(0,0,0,0.05)] md:shadow-none">
        ...
        <div className="hidden md:flex pt-[24px] pb-[20px] px-[24px] border-b-[2.5px] border-[var(--border-color)]">
          <img
            src="/logo.png"
            alt="VALID Logo"
            className="h-[36px] md:h-[40px] object-contain hover:scale-105 transition-transform cursor-pointer"
        ...
        <div className="hidden md:flex items-center justify-between border-b-[2.5px] border-[var(--border-color)] bg-[var(--bg-a)]">
          ...
          >
            <div className="w-[40px] h-[40px] rounded-[12px] border-[2px] border-[var(--border-color)] bg-blue-100 flex items-center justify-center shadow-[3px_3px_0px_var(--border-color)] group-hover:-translate-y-0.5 group-hover:shadow-[4px_4px_0px_#2563EB] group-hover:border-blue-600 transition-all">
              <span
                className="font-black text-[18px] text-blue-600"
            ...
            <div className="flex flex-col">
              <span
                className="font-black text-[14px] text-[var(--text-color)] leading-tight tracking-wide group-hover:text-blue-600 transition-colors truncate max-w-[110px]"
          ...
          <div className="pr-[16px] relative">
            <button
              ...
              className="p-2 relative hover:bg-black/5 dark:hover:bg-white/5 rounded-full transition-colors"
            >
            ...
            {showNotifs && (
              <div className="absolute top-[100%] right-0 w-[280px] bg-[var(--card-bg)] border-[2.5px] border-[var(--border-color)] shadow-[4px_4px_0px_var(--shadow-color)] rounded-[1rem] p-[16px] z-50">
                <div
                  className="font-black text-[12px] uppercase tracking-wider text-[var(--text-color)] mb-3"
                  style={{ fontFamily: "var(--font-body)" }}
                >
                  Notifikasi
                </div>
                <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto">
                  {notifs.length === 0 ? (
                    ...
                    notifs.map((n, i) => (
                      <div
                        key={n.notifId || i}
                        className={`text-[11px] text-[var(--text-color)] border-b border-[var(--border-color)] pb-2 last:border-0 last:pb-0 ${!n.isRead ? "font-bold" : "font-normal"}`}
                      >
                        {n.message}
                      </div>
                    ))
        ...
        <div className="flex-1 flex md:flex-col flex-row w-full justify-start md:px-[16px] md:py-[20px] gap-2 md:gap-[8px] items-center md:items-stretch h-full overflow-x-auto md:overflow-y-auto px-4 py-2 scrollbar-hide">
          {navItems.map((item, idx) => {
            ...
            return (
              ...
              >
                <item.icon
                  className={`w-[20px] h-[20px] md:w-[18px] md:h-[18px] transition-transform ${isActive ? "md:scale-110" : "group-hover:scale-110"}`}
                  strokeWidth={isActive ? 2.5 : 2}
                />
                <span
                  className={`text-[9px] md:text-[12px] uppercase tracking-wider ${isActive ? "font-black" : "font-bold"}`}
                  style={{ fontFamily: "var(--font-body)" }}
                >
                  {item.label}
                </span>
        ...
        <div className="hidden md:flex flex-col p-[24px] border-t-[2.5px] border-[var(--border-color)] bg-[var(--bg-a)]">
          <div
            ...
            className="bg-yellow-300 border-[2px] border-slate-900 rounded-[1rem] p-[16px] shadow-[4px_4px_0px_#0f172a] transform -rotate-1 hover:rotate-0 transition-transform cursor-pointer"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-end gap-1">
                  <span
                    className="font-black text-[32px] text-slate-900 leading-none"
                    style={{ fontFamily: "var(--font-impact)" }}
                  >
                    {user?.reputationPoints || 0}
                  </span>
                  <span
                    className="font-black text-[12px] uppercase text-slate-800 tracking-widest mb-1"
                    style={{ fontFamily: "var(--font-body)" }}
                  >
                    poin
                  </span>
          ...
          >
            ...
            <span
              className="font-black text-[11px] uppercase tracking-widest text-[var(--text-muted)] group-hover:text-red-500 transition-colors"
              style={{ fontFamily: "var(--font-body)" }}
            >
              Keluar
            </span>
