...
export function Step3({ setStep, currentQ, setCurrentQ, sessionId, setScores }: { setStep: (s: number) => void; currentQ: number; setCurrentQ: (q: number) => void; sessionId: string; setScores: (s: any) => void }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      interviewApi.sendAnalysisLog({
        sessionId,
        faceData: { dominantExpression: 'neutral', eyeContactScore: 75, headPostureScore: 80 },
        voiceData: { wordsPerMinute: 120, pauseDuration: 2, volumeVariation: 40 }
  ...
  const handleNext = async () => {
    try {
      await interviewApi.submitAnswer({ sessionId, questionId, answerText: answerText || 'Dummy answer text', durationSeconds: 30 });
      if (currentQ < 5) {
        setAnswerText('');
    ...
    >
      ...
      <div className="w-full md:w-[40%] flex-1 overflow-y-auto bg-[var(--card-bg)] border-t md:border-t-0 md:border-l-[3px] border-[var(--border-color)] flex flex-col px-6 md:px-8 py-6 md:py-9 transition-colors duration-500">
        ...
        <textarea 
          ...
          placeholder="Jawaban kamu akan muncul di sini saat kamu berbicara..."
          style={{ flex: 1, marginTop: "18px", backgroundColor: "var(--bg-b)", border: "2px solid var(--border-color)", borderRadius: "14px", padding: "16px 18px", color: "var(--text-color)", outline: "none", resize: "none", fontFamily: "var(--font-body), 'DM Sans', sans-serif", fontSize: "14px", fontWeight: 500, transition: "background-color 500ms ease, border-color 500ms ease" }} 
        />
