...
import { 
  ...
  Search, Star, CheckCircle2, Briefcase, Eye, Calendar
} from 'lucide-react';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const history = (dashRes.recentReviews || []).map((p: any) => ({
          id: p.portfolioId,
          name: p.title,
          role: p.vocationField,
          dateCompleted: new Date(p.reviewedAt).toLocaleDateString(),
          aiScore: p.myScore || 0, // Mock AI Score since it's not readily available here without extra query
          proScore: p.myScore || 0,
          comment: "Penilaian telah berhasil disubmit ke kandidat." // Default comment since we don't have the original comment readily in dashboard response
        }));
      ...
      <div className="flex-1 h-[calc(100vh-75px)] md:h-screen overflow-y-auto relative pb-[100px] md:pb-[40px]">
        ...
        >
          ...
          <motion.div variants={sectionVariants} className="mb-[40px] flex flex-col lg:flex-row justify-between items-start lg:items-end gap-[24px]">
            <div>
              ...
              <p className="font-bold text-[14px] md:text-[16px] text-slate-600 max-w-[600px] leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>
                Daftar semua kandidat yang sudah kamu ulas. Kamu bisa melihat kembali skor yang diberikan.
              </p>
            </div>
          ...
          <motion.div variants={sectionVariants} className="grid grid-cols-1 gap-[24px]">
            ...
            {!loading && reviews.filter(req => req.name.toLowerCase().includes(search.toLowerCase())).length === 0 && (
              <div className="p-[40px] text-center font-black text-[14px] text-slate-400 uppercase tracking-widest" style={{ fontFamily: 'var(--font-body)' }}>Tidak ada riwayat.</div>
            )}
            {!loading && reviews.filter(req => req.name.toLowerCase().includes(search.toLowerCase())).map((req) => (
              <div key={req.id} className="bg-white border-[3px] border-slate-900 rounded-[1.5rem] p-[24px] shadow-[6px_6px_0px_#0f172a] hover:shadow-[8px_8px_0px_#0f172a] hover:-translate-y-1 transition-all flex flex-col md:flex-row gap-6">
                ...
                <div className="flex flex-col gap-4 min-w-[200px]">
                  <div className="flex items-center gap-4">
                    <div className="w-[56px] h-[56px] rounded-[16px] border-[3px] border-slate-900 bg-yellow-100 flex items-center justify-center shrink-0 shadow-[3px_3px_0px_#0f172a]">
                      <span className="font-black text-[24px] text-yellow-600" style={{ fontFamily: 'var(--font-impact)' }}>{req.name.charAt(0)}</span>
                    </div>
                    <div>
                      <h3 className="font-black text-[18px] text-slate-900 uppercase tracking-tight leading-none mb-1" style={{ fontFamily: 'var(--font-impact)' }}>{req.name}</h3>
                      <div className="font-bold text-[11px] text-slate-500 tracking-widest flex items-center gap-1 uppercase" style={{ fontFamily: 'var(--font-body)' }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="font-bold text-[10px] text-slate-400 uppercase tracking-widest flex items-center gap-1" style={{ fontFamily: 'var(--font-body)' }}>
                ...
                <div className="flex-1 flex flex-col md:flex-row gap-4 border-t-[2.5px] border-dashed border-slate-200 pt-4 md:pt-0 md:border-t-0 md:border-l-[2.5px] md:pl-6">
                  ...
                  <div className="flex items-center gap-6 shrink-0">
                    <div className="flex flex-col items-center">
                      <span className="font-bold text-[10px] text-slate-400 uppercase tracking-widest mb-1" style={{ fontFamily: 'var(--font-body)' }}>Skor AI</span>
                      <div className="w-[48px] h-[48px] rounded-full border-[2.5px] border-slate-200 flex items-center justify-center font-black text-[16px] text-slate-400" style={{ fontFamily: 'var(--font-impact)' }}>
                        {req.aiScore}
                      </div>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="font-bold text-[10px] text-blue-500 uppercase tracking-widest mb-1" style={{ fontFamily: 'var(--font-body)' }}>Skor Kamu</span>
                      <div className="w-[48px] h-[48px] rounded-full border-[2.5px] border-blue-500 flex items-center justify-center font-black text-[18px] text-blue-600 bg-blue-50" style={{ fontFamily: 'var(--font-impact)' }}>
                        {req.proScore}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex-1">
                    <span className="font-bold text-[10px] text-slate-400 uppercase tracking-widest mb-2 block" style={{ fontFamily: 'var(--font-body)' }}>Komentar Kamu</span>
                    <p className="font-bold text-[13px] text-slate-700 italic leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>
                      "{req.comment}"
                    </p>
                  </div>
