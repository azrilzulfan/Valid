...
export function ProReviewRoom() {
  ...
  useEffect(() => {
    if(portfolioId) {
      portfolioApi.getPortfolioById(portfolioId)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      navigate({ to: '/pro/dashboard' }); // fallback
    }
  };
        ...
        <div className="flex-1 flex flex-col gap-[24px]">
          ...
          <div className="bg-white rounded-[1.5rem] border-[3px] border-slate-900 shadow-[6px_6px_0px_#0f172a] p-[24px]">
            <div className="flex items-start gap-4">
               ...
               <div>
                 <h3 className="font-black text-[24px] md:text-[28px] text-slate-900 uppercase tracking-tight leading-none mb-1" style={{ fontFamily: 'var(--font-impact)' }}>{data?.user?.displayName || 'Memuat...'}</h3>
                 <p className="font-bold text-[12px] md:text-[13px] text-slate-500 uppercase tracking-widest mb-3" style={{ fontFamily: 'var(--font-body)' }}>{data?.user?.vocationField || '...'} | {data?.user?.location || '...'}</p>
                 <div className="flex flex-wrap gap-2">
                   {(data?.user?.skills || []).map((skill: string) => (
                     <span key={skill} className="px-2 py-1 bg-slate-100 border-[2px] border-slate-900 rounded-md font-black text-[9px] uppercase tracking-widest text-slate-700" style={{ fontFamily: 'var(--font-body)' }}>{skill}</span>
                   ))}
                 </div>
               </div>
            </div>
          ...
          <div className="bg-white rounded-[1.5rem] border-[3px] border-slate-900 shadow-[6px_6px_0px_#0f172a] overflow-hidden flex flex-col flex-1">
            <div className="bg-yellow-300 px-[24px] py-[16px] border-b-[3px] border-slate-900 flex justify-between items-center">
               ...
               <span className="font-bold text-[10px] uppercase tracking-widest text-slate-800" style={{ fontFamily: 'var(--font-body)' }}>{(data?.portfolios || []).length} Proyek Diunggah</span>
            </div>
            <div className="p-[24px] flex flex-col gap-4 overflow-y-auto max-h-[350px]">
               {(data?.portfolios?.length > 0 ? data.portfolios : data ? [data] : []).map((proj: any) => (
                 <div key={proj._id || proj.title} className="border-[2.5px] border-slate-900 rounded-xl p-4 hover:bg-slate-50 transition-colors group cursor-pointer">
                   <h4 className="font-black text-[14px] text-slate-900 uppercase group-hover:text-blue-600 transition-colors" style={{ fontFamily: 'var(--font-impact)' }}>{proj.title}</h4>
                   <p className="font-bold text-[11px] text-slate-500 uppercase tracking-widest mt-1 mb-2" style={{ fontFamily: 'var(--font-body)' }}>{proj.role} • {new Date(proj.createdAt || Date.now()).getFullYear()}</p>
                   <p className="font-bold text-[12px] text-slate-600 leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>{proj.description}</p>
                   <div className="mt-3 flex flex-wrap gap-2">
                     {(proj.tags || []).map((tag: string) => (
                       <span key={tag} className="text-[10px] bg-blue-100 border-[2px] border-blue-200 text-blue-700 px-2 py-1 rounded-md font-black uppercase tracking-widest" style={{ fontFamily: 'var(--font-body)' }}>{tag}</span>
                     ))}
                   </div>
                 </div>
               ))}
