...
const getCandidateDashboard = async (req, res, next) => {
  try {
    ...
    const avgBehavioralScore = completedSessions.length > 0
      ? Math.round(completedSessions.reduce((sum, s) => sum + (s.overall || 0), 0) / completedSessions.length)
      : null;

    const approvedPortfolios = portfolios.filter(p => p.status === 'approved');
    const avgTechnicalScore = approvedPortfolios.length > 0
      ? Math.round(approvedPortfolios.reduce((sum, p) => sum + (p.verifiedScore || 0), 0) / approvedPortfolios.length)
      : null;
...
const getReviewerDashboard = async (req, res, next) => {
  try {
    ...
    const reviewedSnap = await db.collection('portfolios')
      .where('peerReviews', 'array-contains', { reviewerId: uid })
      .get();

    // Fallback: query manual karena Firestore tidak support nested array query dengan baik
    const allPortfoliosSnap = await db.collection('portfolios')
      .orderBy('submittedAt', 'desc')
      .limit(100)
      .get();
    ...
    const pendingSnap = await db.collection('portfolios')
