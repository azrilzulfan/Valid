      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {PUBLIC_PORTFOLIOS.map((item, idx) => (
            <div key={idx} className="bg-[var(--card-bg)] rounded-[2rem] p-6 md:p-8 border-4 border-[var(--border-color)] shadow-[8px_8px_0px_var(--shadow-color)] hover:-translate-y-2 hover:shadow-[12px_12px_0px_var(--shadow-color)] transition-all">
              <div className="flex items-start gap-6 mb-6">
                {/* Avatar and Score */}
                <div className="flex flex-col items-center gap-3 shrink-0">
                  <div className={`w-20 h-20 rounded-full border-4 border-[var(--border-color)] ${item.color} flex items-center justify-center shadow-[4px_4px_0px_var(--shadow-color)]`}>
                    <span className="text-4xl font-black text-slate-900" style={{ fontFamily: 'var(--font-impact)' }}>{item.initial}</span>
                  </div>
                  <div className="bg-[var(--card-bg)] border-2 border-[var(--border-color)] rounded-xl px-4 py-1 text-center shadow-[2px_2px_0px_var(--shadow-color)]">
                    <div className="text-xl font-black text-blue-600" style={{ fontFamily: 'var(--font-impact)' }}>{item.score}</div>
                ...
                <div className="flex-1">
                  <h3 className="text-2xl font-black uppercase text-[var(--text-color)] tracking-tight mb-1" style={{ fontFamily: 'var(--font-impact)' }}>{item.name}</h3>
