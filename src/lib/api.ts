...
export const authApi = {
  ...
  register: async (data: { uid: string; email: string; displayName: string; vocationField: string }) => {
...
export const usersApi = {
  ...
  updateProfile: async (data: { displayName?: string; vocationField?: string; bio?: string; location?: string; phoneNumber?: string }) => {
    const res = await fetch(`${BASE_URL}/api/users/profile`, {
      method: 'PUT',
      headers: getHeaders(),
      body: JSON.stringify(data),
...
export const portfolioApi = {
  ...
  reviewPortfolio: async (portfolioId: string, data: { technicalAccuracy: number; processDocumentation: number; originality: number; feedback: string }) => {
...
export const interviewApi = {
  ...
  submitAnswer: async (data: { sessionId: string; questionId: string; answerText: string; durationSeconds: number }) => {
  ...
  submitAnalysisLog: async (data: { 
    ...
    const res = await fetch(`${BASE_URL}/api/interview/analysis-log`, {
      method: 'POST',
      headers: getHeaders(),
      body: JSON.stringify(data),
    });
