...
import { Search, ChevronDown, Eye, UserX, UserCheck, X, ChevronLeft, ChevronRight } from 'lucide-react';

const DUMMY_USERS = [
  { id: 1, name: 'Rizky Pratama', email: 'rizky@email.com', role: 'Pencari Kerja', status: 'Aktif', joinDate: '1 Jan 2026', activity: '2 jam lalu' },
  { id: 2, name: 'Budi Santoso', email: 'budi@email.com', role: 'Profesional', verified: true, status: 'Aktif', joinDate: '15 Des 2025', activity: '1 hari lalu' },
  { id: 3, name: 'Siti Rahayu', email: 'siti@email.com', role: 'Profesional', verified: false, status: 'Aktif', joinDate: '20 Nov 2025', activity: '3 hari lalu' },
  { id: 4, name: 'Ahmad Fauzi', email: 'ahmad@email.com', role: 'Pencari Kerja', status: 'Aktif', joinDate: '5 Feb 2026', activity: '5 jam lalu' },
  { id: 5, name: 'Dewi Lestari', email: 'dewi@email.com', role: 'Profesional', verified: true, status: 'Aktif', joinDate: '8 Okt 2025', activity: '2 hari lalu' },
  { id: 6, name: 'Andi Wijaya', email: 'andi@email.com', role: 'Pencari Kerja', status: 'Suspended', joinDate: '12 Mar 2026', activity: '14 hari lalu' },
  { id: 7, name: 'Maya Sari', email: 'maya@email.com', role: 'Pencari Kerja', status: 'Aktif', joinDate: '3 Apr 2026', activity: '30 mnt lalu' },
  { id: 8, name: 'Rudi Hartono', email: 'rudi@email.com', role: 'Admin', status: 'Aktif', joinDate: '1 Jan 2025', activity: '1 jam lalu' },
];
...
export function AdminUsers() {
  ...
  const handleSuspend = (id: number) => {
    setData(prev => prev.map(item => item.id === id ? { ...item, status: 'Suspended' } : item));
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto pb-20">
      <AdminPageHeader 
        ...
        subtitle="ADMIN"
        rightContent={<span className="text-[var(--text-color)] text-[14px] font-black uppercase px-4 py-2 border-[2.5px] border-[var(--border-color)] bg-yellow-300 rounded-xl shadow-[3px_3px_0px_var(--border-color)]" style={{ fontFamily: 'var(--font-body)' }}>2.847 PENGGUNA</span>}
      />
      ...
      <div className="bg-[var(--card-bg)] border-[3px] border-[var(--border-color)] shadow-[6px_6px_0px_var(--shadow-color)] rounded-[20px] p-[16px_20px] flex flex-col md:flex-row gap-[16px] mb-[24px]">
        ...
        <div className="flex gap-[12px] overflow-x-auto pb-2 md:pb-0">
          {['Semua Role', 'Semua Status', 'Terbaru'].map(filter => (
            <button key={filter} className="flex items-center gap-[8px] px-[16px] py-[12px] bg-[var(--bg-a)] border-[2.5px] border-[var(--border-color)] rounded-[14px] text-[13px] font-black uppercase text-[var(--text-color)] hover:-translate-y-0.5 hover:shadow-[3px_3px_0px_var(--border-color)] transition-all shrink-0" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      <div className="flex gap-[12px] mb-[32px] overflow-x-auto pb-2 md:pb-0">
        {['SEMUA (2847)', 'PENCARI KERJA (2640)', 'PROFESIONAL (186)', 'ADMIN (21)'].map(tab => {
          ...
          const isActive = activeTab === (tab.includes('SEMUA') ? 'SEMUA' : tabName.replace('_', ' '));
          return (
            <button
              key={tab}
              onClick={() => setActiveTab(tab.includes('SEMUA') ? 'SEMUA' : tabName.replace('_', ' '))}
          ...
          <div className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <div className="p-[24px_32px] border-b-[3px] border-[var(--border-color)] bg-yellow-300 flex justify-between items-center shrink-0">
                ...
                <button onClick={() => setDetailPanel(null)} className="w-[36px] h-[36px] flex items-center justify-center rounded-[10px] border-[2.5px] border-slate-900 bg-white hover:-translate-y-0.5 hover:shadow-[3px_3px_0px_#0f172a] text-slate-900 transition-all">
              ...
              <div className="flex-1 overflow-y-auto p-[32px]">
                ...
                <div className="flex items-center gap-[20px] mb-[40px]">
                  <div className="w-[80px] h-[80px] rounded-[20px] border-[3px] border-[var(--border-color)] bg-blue-400 flex items-center justify-center shrink-0 shadow-[4px_4px_0px_var(--shadow-color)]">
                    <span className="text-slate-900 text-[36px] font-black uppercase" style={{ fontFamily: 'var(--font-impact)' }}>{data.find(u => u.id === detailPanel)?.name?.[0]}</span>
                  </div>
                  <div>
                    <h2 className="text-[var(--text-color)] text-[24px] font-black uppercase leading-tight" style={{ fontFamily: 'var(--font-impact)' }}>{data.find(u => u.id === detailPanel)?.name}</h2>
                    <p className="text-[var(--text-muted)] text-[14px] font-bold mb-2" style={{ fontFamily: 'var(--font-body)' }}>{data.find(u => u.id === detailPanel)?.email}</p>
