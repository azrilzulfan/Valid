...
const DUMMY_TRANSACTIONS = [
  { id: 1, date: '25 Mei 2026', desc: 'Review oleh Budi Santoso', amount: -50, balance: 75, type: 'payment' },
  { id: 2, date: '24 Mei 2026', desc: 'Top Up 50 koin', amount: 50, balance: 125, type: 'topup' },
  { id: 3, date: '20 Mei 2026', desc: 'Review oleh Ahmad Fauzi', amount: -60, balance: 75, type: 'payment' },
          ...
          <motion.div variants={sectionVariants as any}>
            ...
            <div className="bg-[var(--card-bg)] border-[2.5px] border-[var(--border-color)] shadow-[4px_4px_0px_var(--shadow-color)] rounded-[20px] overflow-hidden">
              ...
              {DUMMY_TRANSACTIONS.map((trx, i) => (
                <div key={trx.id} className="grid grid-cols-1 md:grid-cols-4 p-[20px_24px] border-b-[2px] border-dashed border-[var(--border-color)] hover:bg-[var(--bg-a)] transition-colors last:border-0 gap-3 md:gap-0 items-center">
                  <div className="font-bold text-[13px] text-[var(--text-muted)]" style={{ fontFamily: 'var(--font-body)' }}>{trx.date}</div>
                  ...
                  <div className="font-black text-[14px] text-[var(--text-color)] col-span-2 flex items-center gap-[12px] uppercase" style={{ fontFamily: 'var(--font-body)' }}>
                    <div className={`w-[12px] h-[12px] rounded-full border-[2px] border-slate-900 shadow-[1px_1px_0px_#0f172a] shrink-0 ${
                      trx.type === 'topup' ? 'bg-yellow-400' : trx.type === 'payment' ? 'bg-blue-500' : trx.type === 'bonus' ? 'bg-purple-500' : 'bg-green-500'
                    }`} />
                    {trx.desc}
                  </div>
                  
                  <div className="flex md:flex-col justify-between md:items-end w-full md:w-auto">
                    <span className="md:hidden font-bold text-[12px] text-[var(--text-muted)]">Saldo: {trx.balance}</span>
                    <div className="text-right flex flex-col items-end">
                      <div className="bg-[var(--card-bg)] border-[2px] border-[var(--border-color)] shadow-[2px_2px_0px_var(--shadow-color)] px-[10px] py-[4px] rounded-lg">
                        <div className={`font-black text-[16px] leading-none ${trx.amount > 0 ? 'text-green-600' : 'text-red-600'}`} style={{ fontFamily: 'var(--font-impact)' }}>
                          {trx.amount > 0 ? '+' : ''}{trx.amount} koin
                        </div>
                      </div>
                      <div className="hidden md:block font-bold text-[11px] text-[var(--text-muted)] mt-[6px]" style={{ fontFamily: 'var(--font-body)' }}>Saldo: {trx.balance}</div>
                    </div>
                  </div>
                </div>
              ))}
