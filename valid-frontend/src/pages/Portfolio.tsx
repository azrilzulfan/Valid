...
export function Portfolio() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Ambil array dari response backend dengan aman
  ...
  // Format slug nama untuk URL profile publik yang bersih dan valid
  const userSlug = user?.displayName
    ? encodeURIComponent(user.displayName.toLowerCase().trim().replace(/\s+/g, "-"))
    : "user";
      ...
      <div className="flex-1 h-[calc(100vh-75px)] md:h-screen overflow-y-auto bg-[var(--bg-a)] relative pb-[100px] md:pb-[40px]">
        ...
        >
          ...
          >
            ...
            <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left">
              <h2
                ...
                style={{ fontFamily: "var(--font-impact)" }}
              >
                {loading ? "..." : user?.displayName || "Pengguna Valid"}
              </h2>
              <p
                className="font-bold text-[13px] md:text-[14px] text-[var(--text-muted)] mt-1"
                style={{ fontFamily: "var(--font-body)" }}
              >
                {user?.vocationField || "Kandidat Umum"} — {user?.location || "Indonesia"}
              </p>
              {user?.bio && (
                <p
                  className="font-medium text-[13px] text-[var(--text-muted)] mt-2 max-w-[500px]"
                  style={{ fontFamily: "var(--font-body)" }}
                >
                  {user.bio}
                </p>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-[16px]">
                <span
                  className={`px-[12px] py-[4px] rounded-full border-[2px] border-[var(--border-color)] font-black text-[10px] uppercase tracking-widest ${
                    user?.badgeLevel === "gold"
                      ? "bg-yellow-300 text-yellow-900"
                      : user?.badgeLevel === "silver"
                        ? "bg-slate-200 text-slate-800"
                        : user?.badgeLevel === "bronze"
                          ? "bg-orange-200 text-orange-800"
                          : "bg-[var(--bg-a)] text-[var(--text-muted)]"
                  }`}
                  style={{ fontFamily: "var(--font-body)" }}
                >
            ...
            >
              ...
              <div className="flex flex-col">
                {interviewHistory.slice(0, 3).map((sess: any, i: number) => {
                  ...
                  return (
                    <div
                      key={sess.id || sess._id || sess.sessionId || i}
                    ...
                    >
                      <div className="w-[60px] sm:w-[80px] shrink-0 text-right pt-[2px]">
                        <div
                          ...
                          style={{ fontFamily: "var(--font-body)" }}
                        >
                          {startedDate.toLocaleDateString("id-ID", {
                            day: "numeric",
                            month: "short",
                          })}
                        </div>
                      ...
                      <div className="flex-1 pb-[8px]">
                        <div className="flex items-baseline gap-1">
                          <span
                            ...
                            style={{ fontFamily: "var(--font-impact)" }}
                          >
                            {sess.overallScore ?? sess.score ?? sess.overall ?? "-"}
                          </span>
                          <span
                        ...
                        <div
                          ...
                          style={{ fontFamily: "var(--font-body)" }}
                        >
                          {sess.vocationField?.toUpperCase() || "UMUM"} ·{" "}
                          {sess.status?.toUpperCase() || "SELESAI"}
                        </div>
                      </div>
                    </div>
                  );
          ...
          <motion.div variants={sectionVariants}>
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-16 gap-2 text-[var(--text-muted)] font-bold">
                <Loader2 className="w-8 h-8 animate-spin text-blue-600" />
                <span>Memuat portofolio teknis...</span>
              </div>
            ) : portfolios.length === 0 ? (
              <div className="text-center py-12 border-[2.5px] border-dashed border-[var(--border-color)] rounded-[1.5rem] bg-[var(--card-bg)] shadow-[4px_4px_0px_var(--shadow-color)]">
                <p className="text-[var(--text-muted)] font-bold mb-4">
                  Kamu belum memiliki proyek di portfolio aktif.
                </p>
                <button
                  onClick={() => setIsAddProjectOpen(true)}
                  className="bg-blue-600 text-white rounded-[1rem] px-[16px] py-[10px] font-black text-[12px] uppercase tracking-wider mx-auto border-[2.5px] border-slate-900 shadow-[3px_3px_0px_#0f172a] hover:-translate-y-0.5 transition-all"
                >
