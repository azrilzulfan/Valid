...
import { 
  ...
  ArrowUpRight, ArrowDownLeft, Wallet, CreditCard, ChevronRight
} from 'lucide-react';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const income = (dashRes.recentReviews || []).map((p: any) => ({
          id: p.portfolioId,
          type: 'income',
          title: `Review Portofolio: ${p.title}`,
          date: new Date(p.reviewedAt).toLocaleString(),
          amount: 10 // Poin tetap
        }));
      ...
      <div className="fixed md:relative bottom-0 left-0 w-full md:w-[260px] h-[75px] md:h-screen bg-[var(--card-bg)] md:border-r-[3px] border-t-[3px] md:border-t-0 border-slate-900 flex md:flex-col z-50 md:z-auto transition-all shrink-0">
        ...
        <div className="hidden md:flex items-center gap-3 py-[20px] px-[24px] border-b-[3px] border-slate-900 bg-[var(--bg-a)]">
          <div className="w-[44px] h-[44px] rounded-[14px] border-[2.5px] border-slate-900 bg-purple-200 flex items-center justify-center shadow-[3px_3px_0px_#0f172a]">
            <span className="font-black text-[20px] text-purple-700" style={{ fontFamily: 'var(--font-impact)' }}>
              {stats?.profile?.displayName?.charAt(0) || 'U'}
            </span>
          </div>
          <div className="flex flex-col">
            <span className="font-black text-[15px] text-[var(--text-color)] leading-tight tracking-wide" style={{ fontFamily: 'var(--font-body)' }}>{stats?.profile?.displayName || 'User'}</span>
      ...
      <div className="flex-1 h-[calc(100vh-75px)] md:h-screen overflow-y-auto relative pb-[100px] md:pb-[40px]">
        ...
        >
          ...
          <motion.div variants={sectionVariants} className="mb-[40px]">
            <div className="inline-flex px-3 py-1 bg-green-100 border-[2.5px] border-slate-900 rounded-full font-black text-[10px] uppercase tracking-widest text-slate-900 mb-[16px] shadow-[2px_2px_0px_#0f172a]" style={{ fontFamily: 'var(--font-body)' }}>
              KAS & SALDO
            </div>
            ...
            <p className="font-bold text-[14px] md:text-[16px] text-slate-600 max-w-[600px] leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>
              Kelola pendapatan koin kamu dari mereview kandidat dan cairkan langsung ke rekening bank.
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-[32px]">
            ...
            <div className="lg:col-span-1 flex flex-col gap-[32px]">
              ...
              <motion.div variants={sectionVariants} className="bg-slate-900 border-[3px] border-slate-900 rounded-[1.5rem] p-[32px] shadow-[8px_8px_0px_#FBBF24] relative overflow-hidden group">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-6">
                    ...
                    <span className="font-bold text-[12px] text-slate-300 uppercase tracking-widest" style={{ fontFamily: 'var(--font-body)' }}>Saldo Aktif</span>
                  </div>
                  
                  <div className="flex items-end gap-2 mb-1">
                    <span className="font-black text-[56px] text-white leading-none tracking-tighter" style={{ fontFamily: 'var(--font-impact)' }}>
                      {loading ? '-' : (stats?.profile?.reputationPoints || 0)}
                    </span>
                    <span className="font-black text-[16px] text-yellow-400 uppercase mb-2" style={{ fontFamily: 'var(--font-body)' }}>Koin</span>
                  </div>
                  
                  <div className="font-bold text-[14px] text-slate-400 mb-8" style={{ fontFamily: 'var(--font-body)' }}>
                    ≈ Rp {loading ? '-' : ((stats?.profile?.reputationPoints || 0) * 100).toLocaleString('id-ID')}
                  </div>
                  
                  <button className="w-full bg-yellow-400 hover:bg-yellow-300 text-slate-900 rounded-[1rem] py-[16px] font-black text-[13px] uppercase tracking-widest border-[3px] border-slate-900 shadow-[4px_4px_0px_#000000] hover:shadow-[6px_6px_0px_#000000] hover:-translate-y-1 transition-all flex items-center justify-center gap-2" style={{ fontFamily: 'var(--font-body)' }}>
                    <CreditCard className="w-5 h-5" /> Tarik Saldo
                  </button>
                </div>
              </motion.div>

              <motion.div variants={sectionVariants} className="bg-white border-[3px] border-slate-900 rounded-[1.5rem] p-[24px] shadow-[6px_6px_0px_#0f172a]">
                <div className="font-bold text-[12px] text-slate-500 uppercase tracking-widest mb-1" style={{ fontFamily: 'var(--font-body)' }}>Pendapatan 7 Hari Terakhir</div>
                <div className="font-black text-[32px] text-slate-900 tracking-tighter mb-6" style={{ fontFamily: 'var(--font-impact)' }}>+320 KOIN</div>
                ...
                <div className="flex justify-between mt-2 px-1">
                  {['S', 'S', 'R', 'K', 'J', 'S', 'M'].map((day, i) => (
                    <span key={i} className={`font-black text-[10px] w-[24px] text-center uppercase tracking-widest ${i === 6 ? 'text-slate-900' : 'text-slate-400'}`} style={{ fontFamily: 'var(--font-body)' }}>{day}</span>
                  ))}
                </div>
              </motion.div>
            ...
            <motion.div variants={sectionVariants} className="lg:col-span-2">
              <h2 className="font-black text-[24px] md:text-[28px] text-slate-900 uppercase tracking-tight mb-[20px]" style={{ fontFamily: 'var(--font-impact)' }}>RIWAYAT TRANSAKSI</h2>
              ...
              <div className="bg-white border-[3px] border-slate-900 rounded-[1.5rem] shadow-[6px_6px_0px_#0f172a] overflow-hidden">
                <div className="divide-y-[3px] divide-slate-900">
                  {loading && (
                    <div className="p-[40px] text-center font-black text-[14px] text-slate-400 uppercase tracking-widest" style={{ fontFamily: 'var(--font-body)' }}>Memuat...</div>
                  )}
                  {!loading && transactions.length === 0 && (
                    <div className="p-[40px] text-center font-black text-[14px] text-slate-400 uppercase tracking-widest" style={{ fontFamily: 'var(--font-body)' }}>Belum ada transaksi</div>
                  )}
                  {!loading && transactions.map((trx) => (
                    <div key={trx.id} className="p-[20px] md:p-[24px] flex items-center justify-between gap-4 hover:bg-slate-50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`w-[48px] h-[48px] rounded-[12px] border-[2.5px] border-slate-900 flex items-center justify-center shrink-0 shadow-[2px_2px_0px_#0f172a] ${trx.type === 'income' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                          {trx.type === 'income' ? <ArrowUpRight className="w-6 h-6" /> : <ArrowDownLeft className="w-6 h-6" />}
                        </div>
                        <div>
                          <div className="font-black text-[16px] text-slate-900 uppercase tracking-tight mb-1" style={{ fontFamily: 'var(--font-impact)' }}>{trx.title}</div>
                          <div className="font-bold text-[11px] text-slate-500 uppercase tracking-widest" style={{ fontFamily: 'var(--font-body)' }}>{trx.date}</div>
                        </div>
                      </div>
                      <div className={`font-black text-[18px] md:text-[20px] ${trx.type === 'income' ? 'text-green-600' : 'text-red-600'}`} style={{ fontFamily: 'var(--font-impact)' }}>
                        {trx.type === 'income' ? '+' : '-'}{trx.amount} <span className="text-[12px] text-slate-500 uppercase">Koin</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="p-4 border-t-[3px] border-slate-900 bg-slate-50 text-center">
                  <button className="font-black text-[11px] text-blue-600 uppercase tracking-widest hover:underline flex items-center justify-center w-full gap-1" style={{ fontFamily: 'var(--font-body)' }}>
