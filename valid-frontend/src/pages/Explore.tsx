...
// FUNGSI: Komponen/Halaman Eksplorasi Portofolio Kandidat Secara Publik terintegrasi dengan Real API
// API YANG DIBUTUHKAN: dashboardApi.getStats, portfolioApi.getPublic (atau via endpoint pencarian portfolio publik)
...
import { dashboardApi, portfolioApi } from "../lib/api"; // Pastikan portfolioApi sudah diexport di api.ts
...
// Daftar warna acak neobrutalism untuk avatar jika user tidak memiliki foto profil
const AVATAR_COLORS = [
  "bg-blue-400",
  "bg-pink-400",
  "bg-green-400",
...
export function Explore() {
  ...
  // 1. Fetch Statistik Platform (Landing / Public Stats)
  useEffect(() => {
    dashboardApi
      .getStats()
      .then((res) => {
  ...
  // 3. Fetch Data Portofolio Publik Berdasarkan Query Pencarian
  const fetchPublicPortfolios = async (query: string) => {
    setIsLoadingPortfolios(true);
    try {
      // Menembak endpoint GET /api/portfolio/public?search=...
      // Jika di api.ts milikmu menggunakan parameter atau nama fungsi berbeda, sesuaikan argumennya
      const res = await portfolioApi.getPublic(query);
      setPortfolios(res.portfolios || res.data || []);
    } catch (err) {
          ...
          <motion.div variants={sectionVariants} className="mb-[32px]">
            ...
            {stats && (
              <div className="flex gap-4 mt-6">
                ...
                <div className="bg-[var(--card-bg)] border-[2px] border-slate-900 shadow-[3px_3px_0px_#0f172a] rounded-xl px-4 py-2">
                  <div
                    className="font-black text-[20px] text-yellow-500"
                    style={{ fontFamily: "var(--font-impact)" }}
                  >
                    {stats.badgesIssued || 0}
                  </div>
                  <div
                    className="font-bold text-[10px] text-slate-500 uppercase tracking-widest"
                    style={{ fontFamily: "var(--font-body)" }}
            ...
            {isLoadingPortfolios ? (
              ...
              <div className="flex flex-col items-center justify-center border-[3px] border-dashed border-[var(--border-color)] rounded-[2rem] p-12 text-center bg-[var(--card-bg)] shadow-[4px_4px_0px_var(--shadow-color)]">
                ...
                <p className="font-medium text-sm text-[var(--text-muted)] mt-2">
                  Coba gunakan kata kunci atau pencarian keahlian yang lain.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-[24px]">
                {portfolios.map((portfolio, idx) => {
                  // Fallback data jika struktur user bersarang atau datar dari database
                  const ownerName =
                    portfolio.userDisplayName || portfolio.ownerName || "Kandidat Valid";
                  const initialLetter = ownerName.charAt(0).toUpperCase();
                  const avatarBgColor = AVATAR_COLORS[idx % AVATAR_COLORS.length];

                  // Kelola keahlian dari array proyek asli atau field user terkait
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col">
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <p
                              ...
                              style={{ fontFamily: "var(--font-body)" }}
                            >
                              {portfolio.vocationField || "Kandidat Umum"}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-[8px] mt-[16px]">
                          ...
                          {tags.length === 0 && (
                            <span
                              className="bg-[var(--bg-a)] border-[2px] border-[var(--border-color)] text-[var(--text-muted)] rounded-full px-[10px] py-[4px] font-black text-[9px] uppercase tracking-widest"
                              style={{ fontFamily: "var(--font-body)" }}
                            >
                              UMUM
                            </span>
                        ...
                        {/* Real Project Title & Description */}
                        ...
                        {/* Action - Mengarahkan ke halaman profil publik kandidat */}
                        <button
                          onClick={() =>
                            window.open(`/portfolio/${portfolio.id || portfolio._id}`, "_blank")
                          }
