    ...
    const fetchData = async () => {
      try {
        ...
        const pending = (pendRes.portfolios || []).map((p: any) => ({
          ...
          status: 'Menunggu',
          message: p.description || "Mohon direview portofolio saya."
        }));
        ...
        const completed = (dashRes.recentReviews || []).map((p: any) => ({
          ...
          status: 'Selesai',
          score: p.myScore,
          message: "Portofolio telah selesai direview."
        }));
        ...
        >
          ...
          <motion.div variants={sectionVariants} className="bg-white border-[3px] border-slate-900 rounded-[1.5rem] shadow-[6px_6px_0px_#0f172a] overflow-hidden">
            <div className="divide-y-[3px] divide-slate-900 flex flex-col">
              ...
              {!loading && requests.filter(req => (activeTab === 'Semua' || req.status === activeTab) && req.name.toLowerCase().includes(search.toLowerCase())).map((req) => (
              ...
              {!loading && requests.filter(req => (activeTab === 'Semua' || req.status === activeTab) && req.name.toLowerCase().includes(search.toLowerCase())).length === 0 && (
