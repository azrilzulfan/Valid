  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Gagal memuat daftar permintaan ulasan:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="flex w-full h-screen bg-[var(--bg-b)] overflow-hidden text-[var(--text-color)] font-sans">
      ...
      <div className="flex-1 h-[calc(100vh-75px)] md:h-screen overflow-y-auto relative pb-[100px] md:pb-[40px]">
        ...
        >
          ...
          <motion.div
            variants={sectionVariants}
            className="flex bg-[var(--bg-a)] border-[3px] border-slate-900 rounded-[1rem] p-1.5 shadow-[4px_4px_0px_var(--shadow-color)] mb-[24px] max-w-fit"
          >
            {["Semua", "Menunggu", "Selesai"].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-6 py-2.5 rounded-[0.5rem] font-black text-[11px] uppercase tracking-widest transition-all ${activeTab === tab ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900" : "text-slate-400 hover:text-[var(--text-color)]"}`}
                style={{ fontFamily: "var(--font-body)" }}
              >
                {tab}
              </button>
          ...
          <motion.div
            variants={sectionVariants}
            className="bg-[var(--bg-a)] border-[3px] border-slate-900 rounded-[1.5rem] shadow-[6px_6px_0px_var(--shadow-color)] overflow-hidden"
          >
            <div className="divide-y-[3px] divide-slate-900 flex flex-col">
              ...
              {!loading &&
                requests
                  .filter(
                    (req) =>
                      (activeTab === "Semua" || req.status === activeTab) &&
                  ...
                  .map((req) => (
                    <div
                      key={req.id}
                      className="p-[24px] flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group"
                    >
                      <div className="flex items-start gap-4 flex-1">
                        <div className="w-[56px] h-[56px] rounded-[16px] border-[3px] border-slate-900 bg-blue-100 flex items-center justify-center shrink-0 shadow-[3px_3px_0px_#0f172a]">
                          <span
                            className="font-black text-[24px] text-blue-600"
                            style={{ fontFamily: "var(--font-impact)" }}
                          >
                            {req.name?.charAt(0)}
                          </span>
                        ...
                        <div className="flex-1">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-2">
                            <h3
                              className="font-black text-[18px] text-[var(--text-color)] uppercase tracking-tight"
                              style={{ fontFamily: "var(--font-impact)" }}
                            >
                              {req.name}
                            </h3>
                            {req.status === "Menunggu" ? (
                              <div
                                className="inline-flex items-center gap-1.5 px-3 py-1 bg-yellow-100 border-[2px] border-yellow-500 text-yellow-700 rounded-full font-black text-[9px] uppercase tracking-widest self-start"
                                style={{ fontFamily: "var(--font-body)" }}
                              >
                              ...
                              <div
                                className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-100 border-[2px] border-green-600 text-green-700 rounded-full font-black text-[9px] uppercase tracking-widest self-start"
                                style={{ fontFamily: "var(--font-body)" }}
                              >
                                <CheckCircle2 className="w-3 h-3" />
                                Selesai Dinilai ({req.score})
                              </div>
                            )}
                          </div>
                          ...
                          <div className="bg-[var(--bg-b)] border-[2px] border-dashed border-slate-300 dark:border-slate-700 rounded-lg p-3 inline-block w-full max-w-2xl">
                            <p
                              className="font-bold text-[12px] text-slate-500 italic leading-relaxed"
                              style={{ fontFamily: "var(--font-body)" }}
                            >
                              "{req.message}"
                            </p>
                      ...
                      <div className="flex md:flex-col items-center justify-end gap-3 shrink-0 mt-4 md:mt-0">
                        {req.status === "Menunggu" ? (
                          <button
                            ...
                            className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-[24px] py-[14px] font-black text-[12px] uppercase tracking-widest border-[3px] border-slate-900 shadow-[4px_4px_0px_var(--shadow-color)] hover:shadow-[5px_5px_0px_var(--shadow-color)] hover:-translate-y-1 transition-all flex items-center gap-2 w-full md:w-auto justify-center"
                            style={{ fontFamily: "var(--font-body)" }}
                          >
                            Buka Ruang Uji <ChevronRight className="w-4 h-4" />
                          </button>
                        ) : (
                          <button
                            onClick={() => navigate({ to: `/pro/review/${req.id}` })}
                            className="bg-[var(--bg-b)] text-[var(--text-color)] hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl px-[24px] py-[14px] font-black text-[12px] uppercase tracking-widest border-[3px] border-slate-900 shadow-[4px_4px_0px_var(--shadow-color)] hover:shadow-[5px_5px_0px_var(--shadow-color)] hover:-translate-y-1 transition-all flex items-center gap-2 w-full md:w-auto justify-center"
                            style={{ fontFamily: "var(--font-body)" }}
                          >
              ...
              {!loading &&
                requests.filter(
                  ...
                  <div className="p-[60px] text-center bg-[var(--bg-b)]/50">
                    <div className="w-[80px] h-[80px] bg-[var(--bg-a)] rounded-full border-[3px] border-slate-300 dark:border-slate-700 mx-auto flex items-center justify-center mb-4 shadow-[4px_4px_0px_var(--shadow-color)]">
                      <MessageSquare className="w-10 h-10 text-slate-300" />
                    </div>
                    <h3
                      className="font-black text-[20px] text-slate-400 uppercase tracking-tighter mb-1"
                      style={{ fontFamily: "var(--font-impact)" }}
                    >
                      KOSONG
                    </h3>
                    <div
                      className="font-bold text-[12px] text-slate-400 uppercase tracking-widest"
                      style={{ fontFamily: "var(--font-body)" }}
                    >
                      Tidak ada berkas portofolio dalam antrean ini
                    </div>
                  </div>
                )}
            </div>
