// HALAMAN: C:\laragon\www\valid-react\src\components\valid\Kontak.tsx
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          >
            ...
            <div className="mb-6">
              <label className="block text-xs uppercase tracking-wider font-black text-[var(--text-muted)] mb-2">Pesan</label>
              <textarea
                rows={4}
                required
...
function Field({ label, type, placeholder }: { label: string; type: string; placeholder: string }) {
  return (
    <div className="mb-6">
      <label className="block text-xs uppercase tracking-wider font-black text-[var(--text-muted)] mb-2">{label}</label>
      <input
        type={type}
        required
...
function InfoRow({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="flex items-center gap-5 p-4 bg-[var(--card-bg)] border-[2px] border-[var(--border-color)] shadow-[4px_4px_0px_var(--shadow-color)] rounded-2xl hover:border-slate-400 dark:hover:border-slate-700 transition-colors duration-300 group">
      ...
      <div>
        <div className="text-[10px] uppercase tracking-widest text-[var(--text-muted)] font-bold mb-0.5">{label}</div>
