      ...
      <div className="flex-1 h-[calc(100vh-75px)] md:h-screen overflow-y-auto relative pb-[100px] md:pb-[40px]">
        ...
        >
          ...
          >
            <div>
              <div
                className="inline-flex px-3 py-1 bg-yellow-100 border-[2.5px] border-slate-900 rounded-full font-black text-[10px] uppercase tracking-widest text-slate-900 mb-[16px] shadow-[2px_2px_0px_#0f172a]"
                style={{ fontFamily: "var(--font-body)" }}
              >
                EKSPLORASI KANDIDAT
              </div>
              ...
              <p
                ...
                style={{ fontFamily: "var(--font-body)" }}
              >
                Jelajahi profil pencari kerja dan berikan *Review Sukarela* untuk membantu mereka
                berkembang tanpa memungut koin.
              </p>
            </div>
          ...
          >
            {CANDIDATES.filter(
              ...
              <div
                key={candidate.id}
                className="bg-white border-[3px] border-slate-900 rounded-[1.5rem] p-[24px] shadow-[6px_6px_0px_#0f172a] hover:-translate-y-1 hover:shadow-[8px_8px_0px_#0f172a] transition-all duration-300 flex flex-col group"
              >
                {/* Header */}
                <div className="flex justify-between items-start mb-[20px]">
                  <div
                    className={`w-[64px] h-[64px] rounded-[16px] border-[2.5px] border-slate-900 ${candidate.bg} flex items-center justify-center shadow-[3px_3px_0px_#0f172a] group-hover:scale-105 transition-transform`}
                  >
                    <span
                      className={`font-black text-[28px] ${candidate.text}`}
                      style={{ fontFamily: "var(--font-impact)" }}
                    >
                      {candidate.avatar}
                    </span>
                  ...
                  <div className="flex items-center gap-1.5 px-3 py-1 bg-yellow-50 border-[2px] border-yellow-400 rounded-full">
                    ...
                    <span
                      className="font-black text-[10px] text-yellow-700 tracking-widest"
                      style={{ fontFamily: "var(--font-body)" }}
                    >
                ...
                <div className="mb-[20px]">
                  <h3
                    className="font-black text-[22px] text-slate-900 uppercase tracking-tight leading-none mb-1"
                    style={{ fontFamily: "var(--font-impact)" }}
                  >
                    {candidate.name}
                  </h3>
                  <p
                    className="font-bold text-[12px] text-slate-500 uppercase tracking-widest mb-3"
                    style={{ fontFamily: "var(--font-body)" }}
                  >
                    {candidate.role}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {candidate.skills.map((skill, i) => (
                      <span
                        key={i}
                        className="px-2 py-1 bg-slate-100 border-[2px] border-slate-200 rounded-md font-bold text-[9px] uppercase tracking-widest text-slate-600"
                        style={{ fontFamily: "var(--font-body)" }}
                      >
                        {skill}
                      </span>
                ...
                <div className="mt-auto flex flex-col sm:flex-row gap-3 pt-4 border-t-[2.5px] border-dashed border-slate-200">
                  <button
                    onClick={() => navigate({ to: `/p/${candidate.username}` })}
                  ...
                  <button
                    onClick={() => navigate({ to: `/pro/review/${candidate.id}` })}
