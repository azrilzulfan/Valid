  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-0">
        <div
          className="absolute top-[10%] left-[5%] w-[60vw] h-[60vw] bg-emerald-100/50 dark:bg-emerald-900/10 rounded-full blur-[120px] mix-blend-multiply dark:mix-blend-screen opacity-60 animate-pulse"
          style={{ animationDuration: "9s" }}
        />
        <div
          className="absolute bottom-[10%] right-[10%] w-[50vw] h-[50vw] bg-blue-100/50 dark:bg-blue-900/10 rounded-full blur-[100px] mix-blend-multiply dark:mix-blend-screen opacity-60 animate-pulse"
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-wrap items-end justify-between gap-8 mb-20">
          ...
          <div className="p-6 bg-[var(--card-bg)] border-2 border-[var(--border-color)] shadow-[6px_6px_0px_var(--shadow-color)] max-w-md transform rotate-1 transition-all duration-350">
            <p className="text-[var(--text-color)] text-lg md:text-xl font-bold font-body leading-relaxed">
              VALID dibangun bukan cuma buat profil, tapi juga buat dorong perubahan nyata di dunia
              kerja vokasi Indonesia.
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-6 grid-rows-2 gap-6 md:gap-8 auto-rows-fr">
          {/* Bento Cards */}
          <BentoCard
            className="col-span-6 md:col-span-3 row-span-2 min-h-[420px]"
            img={sdgEdu}
            sdg="04"
            title="Pendidikan Berkualitas"
            body="Bukti belajar yang bisa dipercaya jadi standar baru buat hasil pendidikan vokasi."
            color="bg-emerald-600"
          />
          <BentoCard
            className="col-span-6 md:col-span-3 min-h-[260px]"
            img={sdgWork}
            sdg="08"
            title="Pekerjaan Layak"
            body="Lebih banyak pekerja vokasi yang dapat akses ke peluang sesuai keahliannya."
            color="bg-blue-600"
          />
          <BentoCard
            className="col-span-6 md:col-span-2 min-h-[260px]"
            img={sdgEq}
            sdg="10"
            title="Kurangi Kesenjangan"
            body="Validasi yang adil, tanpa lihat asal sekolah."
            color="bg-rose-600"
          />
          <BentoCard
            className="col-span-6 md:col-span-1 min-h-[260px]"
            img={evidence}
            sdg=""
            title="Bukti Nyata"
            body="Bukan janji, tapi kerja."
            color="bg-amber-500"
          />
        </div>
        ...
        <div className="sdg-card mt-8 relative rounded-[2rem] border-[3px] md:border-4 border-[var(--border-color)] shadow-[8px_8px_0px_var(--shadow-color)] overflow-hidden min-h-[300px] transition-all duration-500 hover:-translate-y-2 hover:shadow-[4px_4px_0px_var(--shadow-color)] group bg-[var(--card-bg)]">
          <img
            ...
            alt="Dampak"
            loading="eager"
            className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 opacity-95 dark:opacity-90"
          />
          ...
          <div className="relative p-10 md:p-14 text-[var(--text-color)] max-w-2xl h-full flex flex-col justify-center">
            ...
            <h3
              ...
              style={{ fontFamily: "var(--font-impact)" }}
            >
              Dampak nyata untuk talenta dan industri.
            </h3>
            <p className="text-[var(--text-muted)] text-lg md:text-xl font-medium max-w-lg leading-relaxed">
              Semua bukti kerja, semua validasi ahli, semua hasil wawancara, jadi pondasi profil
...
function BentoCard({
  ...
  return (
    <div
      className={`sdg-card group relative rounded-[2rem] border-[3px] border-[var(--border-color)] shadow-[6px_6px_0px_var(--shadow-color)] overflow-hidden bg-[var(--card-bg)] transition-all duration-500 hover:-translate-y-2 hover:shadow-[4px_4px_0px_var(--shadow-color)] ${className}`}
    >
      <img
        src={img}
        alt={title}
        loading="eager"
        className="absolute inset-0 w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 opacity-95 group-hover:opacity-90 dark:opacity-90 dark:group-hover:opacity-85"
      />
      ...
      {sdg && (
        <div
          className="pointer-events-none absolute -top-8 -right-4 text-[16rem] leading-none text-[var(--text-color)] opacity-[0.06] font-black"
          style={{ fontFamily: "var(--font-impact)" }}
        >
          {sdg}
        </div>
      ...
      <div className="relative h-full flex flex-col justify-end p-8 text-[var(--text-color)]">
        {sdg && (
          <div
            className={`inline-flex items-center self-start px-4 py-1.5 rounded-full ${color} text-white text-xs font-black tracking-wider uppercase mb-4 border-2 border-[var(--border-color)] shadow-[2px_2px_0px_var(--shadow-color)] transform -rotate-2 transition-all duration-350`}
          >
            SDG {sdg}
          </div>
        )}
        <div className="text-3xl md:text-4xl font-extrabold uppercase tracking-tight leading-none mb-3">
          {title}
        </div>
        <p className="text-[var(--text-muted)] text-sm md:text-base font-medium max-w-sm leading-relaxed">
          {body}
        </p>
      </div>
    </div>
